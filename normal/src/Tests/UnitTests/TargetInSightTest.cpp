...
SCENARIO("Animal interacts with target", "[Animal]")
{
    ...
    double const viewDistance(animal.getViewDistance()); // view distance
    double const viewRange(animal.getViewRange()); // view range
    ...
    GIVEN("A new orientation of the animal and a target outside its view field") {
        ...
        // Orient the animal at 31 degrees:
        animal.setHeading(newOrientation);
        // Reposition the target on the x-axis, outside the field of vision
    ...
    GIVEN("A new orientation of the animal and a target inside its view field") {
        ...
        // Create a target in front of the animal:
